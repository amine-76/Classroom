public class Tele {
    ...
    public Tele (float longueur, float hauteur, float largeur) {
        texture_tele = loadImage("texture_tele.jpg");
        texture_ecran = loadImage("texture_ecran.png"); 
        tele = init(longueur,hauteur,largeur);  
    }

    PShape init(float longueur , float hauteur, float largeur){
        ...
        // face pour l'écran 
            ...
            ecran.vertex(-largeurEcran / 2, -hauteurEcran / 2, 0, 0, 0); // Coin supérieur gauche
            ecran.vertex(largeurEcran / 2, -hauteurEcran / 2, -0, 1, 0);  // Coin supérieur droit
            ecran.vertex(largeurEcran / 2, hauteurEcran / 2, -0, 1, 1);   // Coin inférieur droit
            ecran.vertex(-largeurEcran / 2 , hauteurEcran / 2, -0, 0, 1);  // Coin inférieur gauche
            ecran.endShape();
        //transformation
